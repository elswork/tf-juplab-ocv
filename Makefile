...
build:
	...
	--build-arg VERSION=latest-`cat VERSION` . > ../builds/tf-juplab-ocv_`date +"%Y%m%d_%H%M%S"`.txt
tag:
...
build-arm:
	...
	--build-arg VCS_REF=`git rev-parse --short HEAD` \
	--build-arg VERSION=arm32v7-`cat VERSION` . > ../builds/tf-juplab-ocv-arm_`date +"%Y%m%d_%H%M%S"`.txt
tag-arm:
